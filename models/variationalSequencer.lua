...
------------------------------------------------------------------------
assert(not nn.VariationalSequencer, "update nnx package : luarocks install nnx")
local VariationalSequencer, parent = torch.class('nn.VariationalSequencer', 'nn.AbstractSequencer')
local _ = require 'moses'

function VariationalSequencer:__init(module)
   parent.__init(self)
   if not torch.isTypeOf(module, 'nn.Module') then
      error"VariationalSequencer: expecting nn.Module instance at arg 1"
   end
   ...
   if not torch.isTypeOf(module, 'nn.VariationalLSTM') then
      error'Expect only VariationalLSTM now'
   end
...
function VariationalSequencer:updateOutput(input)
   local nStep
   local batch_size
   ...
   -- Note that the VariationalSequencer hijacks the rho attribute of the rnn
   ...
   if self.train ~= false then
      ...
      self.module:sampleNoise(batch_size)
	  ...
	  self.module:resetNoise(batch_size)
...
function VariationalSequencer:updateGradInput(input, gradOutput)
   local nStep
   if torch.isTensor(input) then
...
function VariationalSequencer:accGradParameters(input, gradOutput, scale)
   local nStep
   if torch.isTensor(input) then
      assert(torch.isTensor(gradOutput), "expecting gradOutput Tensor since input is a Tensor")
      assert(gradOutput:size(1) == input:size(1), "gradOutput should have as many elements as input")
...
function VariationalSequencer:accUpdateGradParameters(inputTable, gradOutputTable, lr)
   error"Not Implemented"
end

function VariationalSequencer:training()
   self:forget()
   self._output = {}
...
function VariationalSequencer:evaluate()
   --~ if self.train ~= false then
...
function VariationalSequencer:clearState()
...
VariationalSequencer.__tostring__ = nn.Decorator.__tostring__
